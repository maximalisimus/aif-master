...
function find_images()
{
	...
	unset _img_array
	dialog --backtitle "$VERSION - $SYSTEM ($ARCHI)" --title "$_img_sel_hd" --menu "$_img_sel_bd" 0 0 11 ${_img_mn} 2>${ANSWER}
	wait
...
{
	dialog --backtitle "$VERSION - $SYSTEM ($ARCHI)" --title "$_ldm_greeter_qs_hd" --yesno "$_ldm_greeter_qs_bd" 0 0
	if [[ $? -eq 0 ]]; then
		select_images
	fi
}
